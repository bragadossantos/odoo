            ...
            if kode in allowed_codes:
                for line in product_lines:
                    ...
                    if luxury_group and non_luxury_group and {luxury_group, non_luxury_group}.issubset(line_tax_groups):
                        err_messages.append(_("Invoice %s: line '%s' contains both Luxury-Goods and Non-Luxury-Goods taxes.", move.name or '', line.product_id.display_name or ''))
                    if non_luxury_group and stlg_group and {non_luxury_group, stlg_group}.issubset(line_tax_groups):
                        err_messages.append(_("Invoice %s: line '%s' contains both Non-Luxury-Goods and STLG taxes.", move.name or '', line.product_id.display_name or ''))
                    if stlg_group and stlg_group in line_tax_groups:
                        if not (luxury_group and luxury_group in line_tax_groups):
                            err_messages.append(_("Invoice %s: line '%s' has STLG tax but missing the required Luxury-Goods tax.", move.name or '', line.product_id.display_name or ''))
                    for tax in line.tax_ids:
                        if ((hasattr(tax, 'amount') and float(tax.amount) == 0.0) or (tax.tax_group_id in {zero_group, exempt_group})):
                            err_messages.append(_("Invoice %s: transaction code %s does not allow 0%% (Zero-rated or Exempt) taxes.", move.name or '', kode))
                ...
                for line in product_lines:
                    for tax in line.tax_ids:
                        if hasattr(tax, 'amount') and float(tax.amount) != 0.0:
                            err_messages.append(_("Invoice %s: transaction code %s must always have tax amount 0%%.", move.name or '', kode))
        return err_messages
